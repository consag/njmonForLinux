...
/* njmon version */
#define VERSION "25@11/05/2019" /* version @ day / month / year */
...
#define FUNCTION_END if(debug)fprintf(stderr,"%s ended line %d\n",__func__, __LINE__);
...
void create_socket(char *ip_address, long port, char *hostname, char *utc, char *secretstr)
{
	...
	DEBUG fprintf(stderr, "hello string=\"%s\"\n",buffer);
	mixup(buffer);
...
long size;
	FUNCTION_START;
DEBUG fprintf(stderr, "checking output_char >%ld< to 90pct of output_size, being >%ld<\n",output_char,(long)(output_size * 0.9));
	/*if( output_char > (long)(output_size * 0.95) ) */
	if( output_char > (long)(output_size * 0.9) ) { /* within 10% of the end */
DEBUG fprintf(stderr, "adding another MB\n");
	        size = output_size + ( 1024 * 1024); /* add another MB */
	        output = realloc((void *)output, size);
	        output_size = size;
DEBUG fprintf(stderr, "MB added.\n");
	}
	FUNCTION_END;
}
...
void pstart()
{
	FUNCTION_START;
	buffer_check();
	DEBUG fprintf(stderr, "praw START\n");
	praw("{\n");
	FUNCTION_END;
}
...
void pfinish()
{
	FUNCTION_START;
	buffer_check();
	DEBUG fprintf(stderr, "praw FINISH\n");
...
void psample()
{
	FUNCTION_START;
	buffer_check();
	DEBUG fprintf(stderr, "praw SAMPLE\n");
	praw("  {\n"); /* start of sample */
	FUNCTION_END;
}
...
void psampleend(int comma_needed)
{
	FUNCTION_START;
	buffer_check();
	DEBUG fprintf(stderr, "praw SAMPLEEND\n");
	remove_ending_comma_if_any();
	if(comma_needed)
	        praw("  }\n");  /* end of sample */
	else
...
int i;
	FUNCTION_START;
	DEBUG fprintf(stderr, "praw INDENT\n");
...
void plong(char *name, long long value)
{
	FUNCTION_START;
                if(!name) {
                  DEBUG fprintf(stderr, "plong called with name == null");
                } else {
                DEBUG fprintf(stderr, "provided args name >%s< and value >%ld<\n", name, value);
                }
                DEBUG fprintf(stderr, "initiating buffer_check\n");
	buffer_check();
                DEBUG fprintf(stderr, "done buffer_check\n");
                DEBUG fprintf(stderr, "initiating indent\n");
	indent();
                DEBUG fprintf(stderr, "done indent\n");
                DEBUG fprintf(stderr, "incrementing njmon_long. current value: %ld\n", njmon_long);
	njmon_long++;
                DEBUG fprintf(stderr, "done incrementing njmon_long. current value: %ld\n", njmon_long);
	if(mode == ONE_LEVEL) {
                DEBUG fprintf(stderr, "mode is ONE_LEVEL\n");
	        output_char += sprintf(&output[output_char], "\"%s%s%s_%s\": %lld,\n",
                ...
                DEBUG fprintf(stderr, "mode is NOT ONE_LEVEL\n");
...
void pdouble(char *name, double value)
{
	FUNCTION_START;
                if(!name) {
                  DEBUG fprintf(stderr, "pdouble called with name == null");
                } else {
                DEBUG fprintf(stderr, "provided args name >%s< and value >%ld<\n", name, value);
                }
	indent();
...
void push()
{
	FUNCTION_START;
	buffer_check();
	DEBUG  fprintf(stderr,"XXX size=%ld\n",output_char);
	...
	DEBUG  fprintf(stderr,"YYY size=%ld\n",output_char);
	output[0] = 0;
	output_char = 0;
	FUNCTION_END;
}
...
void filesystems()
{
	...
	while ((fs = getmntent(fp)) != NULL) {
	    if (fs->mnt_fsname[0] == '/') {
		    ...
		    pdouble("fs_full_percent", ((double)vfs.f_blocks - (double)vfs.f_bfree) / 
...
void sys_device_system_cpu(double elapsed, int print)
{
    ...
    /* FUNCTION_START; */
    if (switch_off) {
	DEBUG fprintf(stderr, "DEBUG: switched_off\n");
	return;
    }
...
void make_pid_file()
{
	...
	FUNCTION_START;
	if((fd = open(pid_filename, O_CREAT | O_WRONLY, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH )) < 0) {
		printf("can't open new file for writing fd=%d\n",fd);
		perror("open");
		return; /* no file */
	}
	printf("\nwrite file descriptor=%d\n",fd);
	sprintf(buffer, "%d", getpid() +1 );
	printf("write \"%s\"\n", buffer);
...
void check_pid_file()
{
	...
	FUNCTION_START;
	    ...
	    printf("file descriptor=%d\n",fd);
			...
			close(fd);
                        printf("njmon pid found, but process %d is not running.", pid);
			remove(pid_filename);
			make_pid_file();
	FUNCTION_END;
}
...
#define ORIGINAL_COUNTDELTA(member) ( (q->procs[topper[j].other].member > p->procs[i].member) ? 0 : (p->procs[i].member  - q->procs[topper[j].other].member) )
#define COUNTDELTA(member) ( (q->procs[j].member > p->procs[i].member) ? 0 : (p->procs[i].member  - q->procs[j].member) )
...
char *get_state(char n)
{
	...
	case 'D': return "Waiting-uninterruptible";
	case 'Z': return "Zombie";
	case 'T': return "Stopped";
	case 't': return "Tracing";
	case 'W': return "Paging-or-Waking";
	case 'X': return "Dead";
	case 'x': return "dead";
...
int proc_procsinfo(int pid, int index)
{
	...
	fprintf(stderr, "WARNING: failed to stat file %s", filename);
	return 0;
    }
...
#define pagesize  (1024 * 4)
	...
	FUNCTION_START;
DEBUG fprintf(stderr, "processes with arg >%lf<.\n", elapsed);
	...
	for (j = 0; j < q->processes; j++) {
		...
		cputime = TIMEDELTA(pi_utime, i, j) + TIMEDELTA(pi_stime, i, j);
    ...
    if( max_sorted > 0) { /* don't sort an empty list */
        DEBUG fprintf(stderr, "sorting >%d< entries...\n", max_sorted);
    	qsort((void *) &topper[0], max_sorted, sizeof(struct topper), &cpu_compare);
        DEBUG fprintf(stderr, "sort completed.\n");
    } else {
        DEBUG fprintf(stderr, "nothing to sort: no processes above cpu threshold.\n");
    }
     ...
     * */
    psection("processes");
    DEBUG fprintf(stderr, "json for processes started. nr of entries: %d\n", max_sorted);
	for(entry=0; entry<max_sorted; entry++) {
	        i = topper[entry].index;
	        j = topper[entry].other;
                    DEBUG fprintf(stderr,"i is >%d<\n", i);
                    DEBUG fprintf(stderr,"j is >%d<\n", j);

		/* reporting influxdb and grafana:
 * need to put all processes into one measurement (table)
 * sprintf(str, "process_%ld", (long)p->procs[i].pi_pid); */
		sprintf(str, "process_%s",p->procs[i].pi_comm);
		psub(str);

		/* Note to self: the directory owners /proc/PID is the process owner = worth adding */
		plong("pid", p->procs[i].pi_pid);
		/* Full command line can be found /proc/PID/cmdline with zeros in it! */
		pstring("cmd", p->procs[i].pi_comm);
		plong("ppid", p->procs[i].pi_ppid);
		plong("pgrp", p->procs[i].pi_pgrp);
		plong("priority", p->procs[i].pi_priority);
		plong("nice", p->procs[i].pi_nice);
		plong("session", p->procs[i].pi_session);
		plong("tty_nr", p->procs[i].pi_tty_nr);
		phex("flags", p->procs[i].pi_flags);
		pstring("state", get_state(p->procs[i].pi_state));
#ifndef PRE_KERNEL_2_6_18
		plong("threads", p->procs[i].pi_num_threads);
#endif					
		pdouble("cpu_percent", topper[entry].time / elapsed);
		pdouble("cpu_usr", TIMEDELTA(pi_utime, i, j) / elapsed);
		pdouble("cpu_sys", TIMEDELTA(pi_stime, i, j) / elapsed);
		pdouble("cpu_usr_total_secs", p->procs[i].pi_utime / (double)sysconf(_SC_CLK_TCK));
		pdouble("cpu_sys_total_secs", p->procs[i].pi_stime / (double)sysconf(_SC_CLK_TCK));
		plong("statm_size_kb", p->procs[i].statm_size * pagesize / 1024);
		plong("statm_resident_kb", p->procs[i].statm_resident * pagesize / 1024);
		plong("statm_restext_kb", p->procs[i].statm_trs * pagesize / 1024);
		plong("statm_resdata_kb", p->procs[i].statm_drs * pagesize / 1024);
DEBUG fprintf(stderr, "Start with statm_share_kb (long)\n");
		plong("statm_share_kb", p->procs[i].statm_share * pagesize / 1024);
DEBUG fprintf(stderr, "Done with statm_share_kb\n");
DEBUG fprintf(stderr, "Start with minorfault (double)\n");
DEBUG fprintf(stderr, "elapsed is >%lf<\n", elapsed);
DEBUG fprintf(stderr, "topper i >%d< index pi_minflt is >%lf<\n", i, p->procs[i].pi_minflt);
DEBUG fprintf(stderr, "topper j >%d< other pi_minflt would be >%lf<\n", j, q->procs[j].pi_minflt);
DEBUG fprintf(stderr, "COUNTDELTA >$lf<", COUNTDELTA(pi_minflt));
		pdouble("minorfault", COUNTDELTA(pi_minflt) / elapsed);
DEBUG fprintf(stderr, "Done with minorfault\n");
		pdouble("majorfault", COUNTDELTA(pi_majflt) / elapsed);

		plong("it_real_value", p->procs[i].pi_it_real_value);
		pdouble("starttime_secs", (double)(p->procs[i].pi_start_time) / (double)sysconf(_SC_CLK_TCK));
		plong("virtual_size_kb", (long long)(p->procs[i].pi_vsize)/1024);
		plong("rss_pages", p->procs[i].pi_rss);
		plong("rss_limit", p->procs[i].pi_rsslimit);
		/* NOT INCLUDED AS THEY ARE FOR DEBUGGING AND NOT PERFORMANCE TUNING */
#ifdef PROCESS_DEBUGING_ADDRESSES_SIGNALS
		phex("start_code", p->procs[i].pi_start_code);
		phex("end_code", p->procs[i].pi_end_code);
		phex("start_stack", p->procs[i].pi_start_stack);
		phex("esp_stack_pointer", p->procs[i].pi_esp);
		phex("eip_instruction_pointer", p->procs[i].pi_eip);
		phex("signal_pending", p->procs[i].pi_signal_pending);
		phex("signal_blocked", p->procs[i].pi_signal_blocked);
		phex("signal_ignore", p->procs[i].pi_signal_ignore);
		phex("signal_catch", p->procs[i].pi_signal_catch);
		phex("signal_exit", p->procs[i].pi_signal_exit);
		phex("wchan", p->procs[i].pi_wchan);
#endif
	        /* NOT INCLUDED AS THEY ARE FOR DEBUGGING AND NOT PERFORMANCE TUNING */

		plong("swap_pages", p->procs[i].pi_swap_pages);
		plong("child_swap_pages", p->procs[i].pi_child_swap_pages);
		plong("last_cpu", p->procs[i].pi_last_cpu);
		plong("realtime_priority", p->procs[i].pi_realtime_priority);
		plong("sched_policy", p->procs[i].pi_sched_policy);
		pdouble("delayacct_blkio_secs", (double)(p->procs[i].pi_delayacct_blkio_ticks) / (double)sysconf(_SC_CLK_TCK));
		plong("uid", p->procs[i].uid);
		if(strlen(p->procs[i].username) > 0)
			pstring("username", p->procs[i].username);
	        psubend();
	    }
	psectionend();
    FUNCTION_END;
	...
	while (-1 != (ch = getopt(argc, argv, "?hfm:SMOs:c:kdi:I:Pp:X:x"))) {
		...
		case 's': seconds = atoi(optarg); 
			  if(seconds < 1) 
				seconds = 1;
			  break;
		case 'c': maxloops = atoi(optarg); 
			  break; 
		case 'k': check_pid_file(); 
			  break;
		case 'd': debug++; 
			  break;
#ifndef NOREMOTE
	...
	if(file_output) {
		...
		get_localtime();
	        sprintf( filename, "%s_%02d%02d%02d_%02d%02d.json",
	                         ...
	                         tim->tm_hour,
	                         tim->tm_min);
DEBUG fprintf(stderr, "freopen file for stdout.\n");
	        ...
	        sprintf( filename, "%s_%02d%02d%02d_%02d%02d.err",
	                         ...
	                         tim->tm_hour,
	                         tim->tm_min);
DEBUG fprintf(stderr, "freopen file for stderr\n");
...
DEBUG fprintf(stderr, "fflush null\n");
	fflush(NULL);
...
DEBUG fprintf(stderr, "malloc for %d \n",output_size);
	...
	execute_start = current_time;
	execute_end = current_time;
	...
	for (loop = 0; maxloops == -1 || loop < maxloops; loop++) {
		...
		if(loop > 0) /* don't sleep on first loop as we slept before the loop */
			sleep(quick_sleep);
		...
		if(loop != 0) {  /* don't calulate this on the first loop */
			accumalated_delay += (sleep_end - sleep_start - (double)quick_sleep)
	...
	*/
		psection("njmontime");
		 ...
		 pdouble("njmon_sleep_time", sleep_end - sleep_start);
		 pdouble("njmon_execute_time", execute_end - execute_start);
		 pdouble("njmon_accumalated", accumalated_delay);
		 plong("njmon_quick_sleep", quick_sleep);
		psectionend();
#endif /* timers */
		execute_start = (double)tv.tv_sec + ((double)tv.tv_usec * 1.0e-6);
		...
		gpfs_data(elapsed);
#endif /* NOGPFS */
		if(proc_mode) processes(elapsed);
		DEBUG praw("Sample");
